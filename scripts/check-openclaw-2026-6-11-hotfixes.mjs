...
const checks = [
  ...
  },
  {
    ...
    assertions: [
      ...
      contains("[hotfix][auto-topic-label] boundary persisted", "v3 persistent /new boundary record"),
      contains("[hotfix][auto-topic-label] boundary persist failed", "v3 persistent boundary error log"),
      contains("[hotfix][auto-topic-label] decision", "v3 auto-topic decision diagnostics"),
      contains("[hotfix][auto-topic-label] renamed topic", "v3 rename visible log"),
      contains("[hotfix][auto-topic-label] failed:", "v3 rename failure visible log"),
      contains("messageCache: createTelegramMessageCache(", "v3 boundary lookup in-place cache instance"),
      contains("hotfix v4: only the first user message after the boundary", "v4 first-only relabel marker"),
      contains("compareTelegramTopicLabelMessageIds(node.messageId, params.messageId) >= 0", "v4 prior-message window bound"),
    ],
  },
  {
    id: "conversation-label-prompt-inline",
    gate: "required",
    locate: (files) => findOneJs(files, "conversation label generator bundle", [
      "conversation-label-generator",
      "prepareModelForSimpleCompletion",
      "generateConversationLabel",
    ]),
    assertions: [
      contains("hotfix: conversation-label-prompt-inline", "inline label prompt marker"),
      contains("Topic label:", "inline label prompt user-content instruction"),
      contains("const firstLine = rawText.split(", "label first-line sanitize"),
    ],
  },
  {
    id: "conversation-label-model-auth-fallback",
    gate: "required",
    locate: (files) => findOneJs(files, "conversation label generator bundle", [
      "conversation-label-generator",
      "prepareModelForSimpleCompletion",
    ...
    assertions: [
      contains("hotfix: conversation-label-model-auth-fallback", "label model auth fallback marker"),
      contains("resolveConversationLabelModelCandidates", "label model candidate resolver"),
      contains("generateConversationLabelWithModel", "per-model label attempt helper"),
      contains("agents?.defaults?.model?.fallbacks", "config fallback chain source"),
      contains("requireApiKey(runtimeAuth, candidate.provider)", "per-candidate auth requirement"),
    ],
  },
  {
