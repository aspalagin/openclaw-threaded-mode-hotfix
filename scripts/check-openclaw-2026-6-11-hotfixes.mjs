...
const expectedVersion = "2026.6.11";
...
function main() {
  ...
  const results = [
    ...
    check(files, "telegram-dm-topic-threading-send", (all) => findOne(all, "Telegram send bundle", [
      ...
      (content) => contains(content, "buildTelegramThreadParams", "upstream thread params builder"),
      (content) => contains(content, "resolveForumLaneKey", "upstream forum lane resolver"),
      (content) => contains(content, "direct_messages_topic_id", "upstream direct-topic id handling"),
    ]),
    check(files, "telegram-auto-topic-label-after-new", (all) => findOne(all, "Telegram bot bundle", [
      "generateTelegramTopicLabel",
      "isTelegramSessionBoundaryCommandText",
      "editForumTopic",
    ]), [
      (content) => contains(content, "hotfix: telegram-auto-topic-label-after-new", "auto-topic /new hotfix marker"),
      (content) => contains(content, "resolveTelegramSessionBoundaryTopicLabelTail", "session-boundary tail parser"),
      (content) => contains(content, "isFirstTelegramUserMessageAfterSessionBoundary", "first user message after /new detector"),
      (content) => contains(content, "let isFirstUserMessageAfterSessionBoundary = false;", "first-after-boundary flag"),
      (content) => contains(content, "session boundary lookup error", "session-boundary lookup error log"),
      (content) => contains(content, "const shouldAutoTopicLabel = isDmTopic && ((!isCurrentSessionBoundaryCommand && isFirstTurnInSession) || isFirstUserMessageAfterSessionBoundary || Boolean(sessionBoundaryTopicLabelTail.trim()));", "expanded auto-topic condition"),
      (content) => contains(content, "sessionBoundaryTopicLabelTail || ctxPayload.RawBody || ctxPayload.Body || \"\"", "topic label source tail fallback"),
    ]),
