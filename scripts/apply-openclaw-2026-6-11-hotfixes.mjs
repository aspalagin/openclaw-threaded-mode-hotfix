...
function patchTelegramDmTopicSentMessageCache(source) {
  let next = source;
  // 2026.6.11 has upstream-equivalent DM topic session and delivery fixes.
  // Keep the directMessagesTopicId fallback for packages that still need it.
  next = next.replace(
...
function patchTelegramDmTopicSend(source) {
  // 2026.6.11: accepted upstream. buildTelegramThreadParams sends
  // message_thread_id for DM scope and resolveForumLaneKey handles
  // direct_messages_topic_id, so no portable dist patch is needed.
  return source;
}
